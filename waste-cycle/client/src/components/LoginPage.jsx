...
// Removed: import type { User, UserRole } from '../App';
// Removed: interface LoginPageProps { ... }

export function LoginPage({ onLogin, onBack }) { // Removed type annotation for props
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [isAdmin, setIsAdmin] = useState(false);

  const handleLogin = (e) => { // Removed type annotation
    e.preventDefault();
    
    const mockUser = { // Removed type annotation
      id: '1',
      email: email,
      name: 'สมชาย เกษตรกร',
